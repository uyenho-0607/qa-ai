...
|---|---|---|
| `bo` | Playwright, desktop | `.claude/domain/otc-bo.md` |
| `bo-mv` | Playwright, 390×844 | `.claude/domain/otc-bo.md` |
| `ios` / `android` | mobile MCP | `.claude/domain/otc-mobile.md` |
| `app-web` | Playwright | `.claude/domain/otc-mobile.md` |

Rules spanning both surfaces — password policy, OTP, statuses, decimal precision — are in
`.claude/domain/otc-shared.md`.
...
- Only Android is verified today; no iOS build exists yet
  (`.claude/steering/mobile-mcp-rule.md` § Platform reality). An `ios` target with no device present is
...
Follow `.claude/steering/playwright-rule.md`.
...
Follow `.claude/steering/mobile-mcp-rule.md`.
...
- **There is no network interception.** The `mobile` server observes no traffic. Collect instead: the backend
  ...
  `.claude/steering/mobile-mcp-rule.md`.
...
`.claude/locator-cache.json` § `api` covers the symptom, say so — an unverifiable classification is stated,
never invented.
...
native frame holds no in-frame label. Naming and paths: `.claude/skills/capture-evidence/SKILL.md`
§ File Naming.
