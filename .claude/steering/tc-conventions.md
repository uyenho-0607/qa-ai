...
Format and field order: `.claude/skills/generate-tcs/TEMPLATE.md` § `**Name:**`.

Approved Module, Sub-module, and Feature names: `.claude/domain/tc-naming-ref.md`. Copy them verbatim — that file is the authority for spelling. Extraction command: `generate-tcs/SKILL.md` § 3.1.
...
- Place platform in **Configuration** — must match an existing Testmo config name. The names for this project are in `.claude/steering/testmo.md` § Configurations by Project; where that file has no row for the project in hand, fetch via `testmo_list_configs`.
...
Block format, field set, file header, ER bullet prefixes, and block separator: `.claude/skills/generate-tcs/TEMPLATE.md`.
