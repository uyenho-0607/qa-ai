...
credentials come from `.claude/steering/project-config.md` § Environment, and every stack-specific value
...
`jq '.["{section}"].screens | keys' .claude/locator-cache.json` to list the screens, then
`jq '.["{section}"].screens.{screen}' .claude/locator-cache.json` for the one in hand.
