...
Targets: `ios`, `android` (mobile MCP, per `.claude/steering/mobile-mcp-rule.md`), `app-web` (Playwright, the
Expo web build, per `.claude/steering/playwright-rule.md`). Domain: `.claude/domain/otc-mobile.md`; login
runbook `.claude/domain/login-flow.md`. Cached locators: `.claude/locator-cache.json` § `bfg-otc-app`.

App under test: `APP_PACKAGE` in `.claude/steering/project-config.md` § Environment.
