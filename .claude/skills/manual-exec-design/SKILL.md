...
- **Reads:** 
  `.claude/steering/project-config.md` § Environment, § Platforms, § Producers — extract, never whole:
  `awk '/^## /{p=/^## (Environment|Platforms|Producers)$/} p' .claude/steering/project-config.md`
  the platform packs § Platforms names — § Targets, § Observables and § Stack quirks by heading
  `.claude/steering/tc-exec-classify.md`
  `.claude/locator-cache.json`
  `.claude/domain/flows.md` or the platform domain files
  `TASK/base/jira.md`
  `TASK/base/tc.md`
...
- **Writes:** 
  ...
  Verified locators to `.claude/locator-cache.json`
...
Read `.claude/steering/tc-exec-classify.md` and apply it. Move non-addressable TCs to Skipped with reasons. For remaining TCs:
...
- Act on what comes back:
   ...
   - Merge every returned locator patch into `.claude/locator-cache.json` yourself.
