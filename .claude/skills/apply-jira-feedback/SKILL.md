...
Dispatch the `testmo-collector` agent for `{KEY}`, then read `tasks/{KEY}/base/tc.md`. No Agent tool in context → invoke the `collect-testmo-cases` skill FULLY with `{KEY}`, `save`, `no-gate`. The `update` rows read steps and expected results from that file before rewriting.
...
Classify each numbered item using the action table in `.claude/steering/tc-feedback-actions.md`, then map it to the case ids it touches.
