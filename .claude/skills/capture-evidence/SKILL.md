...
- **Reads:** `project-config.md` § Platforms — extract, never whole: `awk '/^## /{p=/^## (Platforms)$/} p' .claude/steering/project-config.md`; `capture-mechanics.md`; the resolved platform pack; the target's driver rule and capture mechanics file for its Group
...
Read `.claude/steering/capture-mechanics.md`. For each target in `targets`, resolve its pack from `project-config.md` § Platforms, load both the driver rule and the capture mechanics file for its Group per `capture-mechanics.md`, and read the pack's § Targets, § Label overlay, § Preflight, and § Stack quirks sections.
