...
- **Writes:** `tasks/{KEY}/exec/exec.md` (result lines, in place), `tasks/{KEY}/exec/report.md`, evidence to `tasks/{KEY}/exec/evidence/` — the paths `.claude/steering/project-config.md` § Folder Structure fixes
...
**Loads, at Phase 2 and not before:** `.claude/docs/lessons.md`, `.claude/steering/capture-mechanics.md`,
...
Checkpoint Evidence row carries — then the frame is verified, per `.claude/steering/capture-mechanics.md`. No
...
file per `.claude/steering/capture-mechanics.md`, then verify it there. Write no `.md` beside it — the elapsed
...
6. A reusable lesson the run turned up — a locator that moved, a fixture that lies, an env quirk that cost a
   re-run → append one bullet to `.claude/docs/lessons.md` with `{KEY}` and the date. A lesson a rule file
