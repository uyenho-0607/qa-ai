...
- **Reads:**
 ...
 `.claude/steering/project-config.md` § Environment, § Platforms, § Folder Structure — extract, never whole:
 `awk '/^## /{p=/^## (Environment|Platforms|Folder Structure)$/} p' .claude/steering/project-config.md`
 the platform packs `project-config.md` § Platforms names — the enabled platform's pack loads whole
 `.claude/docs/lessons.md`, `.claude/steering/capture-mechanics.md`, and the driver and capture files for each
 platform kind in play — the Phase 2 gate lists them
 `.claude/locator-cache.json`, the endpoint section named in the pack § Observables, for a backend check only — that section alone, never whole
- **Writes:**
 ...
 `.claude/docs/lessons.md` (Phase 3 append)
...
1. **Registry & Header Read**:
   - Read active platform definitions from `.claude/steering/project-config.md` § Platforms.
...
2. **Evidence Destination**:
   ...
   - If not → derive from `Evidence mode`:
     ...
     - `screenshot`: invoke the `scaffold-evidence-doc` skill with `{KEY}`. Record the returned URL and doc id.
...
`.claude/steering/capture-mechanics.md`
- Group = `web`: `.claude/steering/capture-web.md`, `.claude/steering/playwright-rule.md`
- Group = `device`: `.claude/steering/capture-device.md`, `.claude/steering/maestro-rule.md`
...
3. **Assert**: Verify `Expected` assertions against pack observables (not screenshots).
   ...
   - **On Assertion Failure**:
     ...
     2. Perform backend API check if endpoint is named in assertion. Endpoints and their bearer token come
        ...
        never the file: `jq '.{section}' .claude/locator-cache.json`.
...
- **`normal` Mode**: Invoke the `capture-evidence` skill with `targets={active platforms}`, `type={the group's Type, lowercased}`, `stem={stem}`, `dest=tasks/{KEY}/exec/evidence/`, `element={the asserted element's id=/desc=/text=}`, `label={what is being verified}`, and `annotation={annotations}` from the header — per group after the group finishes testing.
...
3. **Generate Report**: Write `tasks/{KEY}/exec/report.md` from `REPORT.md`. Its Evidence column and any
   ...
   `exec.md`.
4. **Lessons Learned**: Append reusable execution lessons to `.claude/docs/lessons.md`
...
3. **File each chosen candidate**: invoke the `report-bug` skill, one per candidate, with the full payload —
