...
1. Invoke the `jira-retriever` skill with `{KEY} save` — plus `skip-figma` when the caller passed `skip-figma` or an explicit Figma URL. Follow it fully. Output lands in `tasks/{KEY}/base/`.
2. A Figma URL in the args wins over the ticket's own links → invoke the `figma-retriever` skill with that URL and `{KEY}`.
