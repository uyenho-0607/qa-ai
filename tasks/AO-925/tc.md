...
**State:** all 65 cases are `Done` (state_id 4).
**Note:** no case carries a priority or automation value — every case is unset on both fields.
...
### By platform under test

| Platform | Cases |
|---|---|
| Back Office only | 52 |
| Mobile app only | 5 |
| BO + mobile app both required | 6 |
| BO + BE/audit-log access required | 2 |

## Case Groups

| Group | Cases | Automated | Can Automate | Pattern |
|---|---|---|---|---|
| A. Mobile-to-BO record reflection | 3 | 0 | 0 | add / edit / delete on mobile, verified in BO |
| B. Tab presence and scope | 3 | 0 | 0 | tab visible, both sections load, no create/edit/delete |
| C. Role permissions | 3 | 0 | 0 | Maker / Checker / Admin visibility of Disable+Enable |
| D. Bank Accounts list | 3 | 0 | 0 | empty state / columns+masking / sort order |
| E. Bank Account Details drawer | 4 | 0 | 0 | fields / unmasked number / blank optionals / Deleted read-only |
| F. Bank Account Disable | 6 | 0 | 0 | list entry / drawer entry / modal copy / cancel / confirm / pending withdrawal |
| G. Bank Account Enable | 7 | 0 | 0 | list entry / drawer entry / modal copy / cancel / confirm / limit blocked / limit not counting Disabled |
| H. Crypto Addresses list | 3 | 0 | 0 | empty state / columns+masking+network / sort order |
| I. Crypto Address Details drawer | 4 | 0 | 0 | fields / unmasked address / memo hidden / Deleted read-only |
| J. Crypto Address Disable | 6 | 0 | 0 | same six variants as group F |
| K. Crypto Address Enable | 6 | 0 | 0 | same six variants as group G, limit 20 |
| L. Audit log | 2 | 0 | 0 | Disable event / Enable event |
| M. API failure handling | 2 | 0 | 0 | Disable fails / Enable fails |
| N. Concurrency | 3 | 0 | 0 | two admins race at the limit, or on the same record |
| O. Balance Approvals — disabled destination | 3 | 0 | 0 | approve blocked / reject succeeds / no auto-reject |
| P. Re-add prevention and app reflection | 4 | 0 | 0 | BO disable reaches app / bank blocked / crypto blocked / deleted re-add allowed |
| Q. Fiat withdrawal destination picker | 2 | 0 | 0 | disabled badge non-selectable / re-enabled selectable |
| R. Crypto withdrawal destination picker | 1 | 0 | 0 | disabled non-clickable |

---

## Full Case Details

### A. Mobile-to-BO record reflection (3 cases)

#### TC-151414 · Members – Details – Withdrawal account record visible in BO after add on mobile
**Priority:** unset | **Automation:** unset | **State:** Done | **Folder:** Members (128)
...
|---|--------|-----------------|
| 1 | Locate the member in BO and open their Withdrawal Account section | 1. Newly added bank account record is visible in BO<br>2. All field values match what was submitted on the mobile form<br>3. Status shows as Active |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Locate the edited bank account record in BO | 1. Updated label is reflected in BO<br>2. Updated currency list is reflected in BO<br>3. Other fields remain unchanged |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Locate the deleted bank account record in BO | 1. Record is still visible in BO (retained for audit)<br>2. Status shows as Deleted<br>3. Date Deleted timestamp is displayed |

**Test Data:** none

---

### B. Tab presence and scope (3 cases)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the tabs available on the Member Details page | 1. A "Withdrawal Accounts" tab is visible alongside other existing tabs<br>2. No errors occur |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the Withdrawal Accounts tab for any Create / Add / New button | 1. No Create, Add, or New button is present on the Withdrawal Accounts tab |
| 2 | Open the action menu and detail drawer for a bank account record and a crypto address record in each status (Active, Disabled, Deleted) | 1. No Edit or Delete action is available in any action menu or detail drawer for any record in any status |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Click on the "Withdrawal Accounts" tab | *(none stated)* |
| 2 | Observe the page content | 1. The Withdrawal Accounts page loads successfully<br>2. "Bank Accounts" section is visible at the top<br>3. "Crypto Addresses" section is visible below Bank Accounts<br>4. Each section heading is clearly labelled |

**Test Data:** none

---

### C. Role permissions (3 cases)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Open any member's Withdrawal Accounts tab | *(none stated)* |
| 2 | Open the action menu for an Active bank account record and an Active crypto address record | 1. No Disable or Enable action is available in any action menu for bank account or crypto address records<br>2. The Withdrawal Accounts tab and all records are visible in read-only mode |

**Test Data:** none

> ⚠️ **Conflicts with the ticket.** BR-1 states Disable/Enable are "available to any Back Office role with Member Details access". This case asserts the Maker role must *not* see them. Resolve before executing.
...
> ⚠️ **Conflicts with the ticket.** Same conflict as TC-161965 — BR-1 grants the actions to any role with Member Details access.

---

#### TC-191123 · Members – Details – Admin role CAN see Disable and Enable actions on withdrawal account records
**Priority:** unset | **Automation:** unset | **State:** Done | **Folder:** Members (128)
*Added 2026-08-25 in response to reviewer feedback on AO-1087.*
...
|---|--------|-----------------|
| 1 | Open any member's Withdrawal Accounts tab | *(none stated)* |
| 2 | Open the action menu for an Active bank account record | 1. A "Disable" action option is visible and clickable |
| 3 | Open the action menu for an Active crypto address record | 1. A "Disable" action option is visible and clickable |
| 4 | Open the action menu for a Disabled bank account record | 1. An "Enable" action option is visible and clickable |
| 5 | Open the action menu for a Disabled crypto address record | 1. An "Enable" action option is visible and clickable |

**Test Data:** none

---

### D. Bank Accounts list (3 cases)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the Bank Accounts section | 1. The section displays: "This member has no bank accounts."<br>2. No table rows or loading indicators are shown |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the Bank Accounts section table columns | 1. The following columns are visible: Bank Name, Account Holder Name, Account Number / IBAN (masked), Currency, Status, Created Date<br>2. Column headers match the specified labels exactly |
| 2 | Observe the Account Number / IBAN column in the Bank Accounts list | 1. Account Number / IBAN is displayed masked — only the last 4 digits are visible (e.g. **** **** **** 1234)<br>2. The full account number is not visible in the list |
| 3 | Observe the Currency column for a bank account linked to multiple currencies | 1. The Currency column displays all currency codes linked to that bank account (e.g. USD, HKD)<br>2. No currencies are truncated or hidden |

**Test Data:** a bank account linked to more than one currency
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the sort order in the Bank Accounts list | 1. Active records appear at the top, sorted by Created Date descending (most recent first)<br>2. Disabled records appear after all Active records, sorted by Created Date descending<br>3. Deleted records appear last, sorted by Created Date descending |

**Test Data:** none

---

### E. Bank Account Details drawer (4 cases)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the Account Number / IBAN field in the detail drawer | 1. The full unmasked Account Number / IBAN is shown<br>2. This is different from the masked format (last 4 digits only) shown in the list view |

**Test Data:** none

> ⚠️ The case title and body say the unmasked number is "visible to Maker role only". BR-3 states the full number is shown on this view without restricting it to one role. Resolve the scope before executing.
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe optional fields in the detail drawer | 1. Optional fields not filled in by the member show as "-" |

**Test Data:** a bank account saved without Address Line 2, Bank Code / Routing Number, Account Type
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Click on a Deleted bank account record in the list | 1. The detail drawer opens successfully<br>2. All fields are visible in read-only mode<br>3. No Disable or Enable buttons are present<br>4. Status field shows "Deleted" |

**Test Data:** none

---

### F. Bank Account Disable (6 cases)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Locate an Active bank account in the list | *(none stated)* |
| 2 | Open the action menu (e.g. overflow "..." button) for that record | 1. A "Disable" action option is visible |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the action buttons available in the detail drawer | 1. A "Disable" button is visible and clickable |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Click "Cancel" on the Disable confirmation modal | 1. The modal closes<br>2. The bank account remains in Active status<br>3. No changes are made |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Click Disable on the Active bank account that has a linked Pending withdrawal | *(none stated)* |
| 2 | Confirm the Disable action in the modal | *(none stated)* |
| 3 | Observe the bank account status and the Pending withdrawal status in Back Office | 1. The Disable action succeeds without error<br>2. Success message: "This bank account has been disabled."<br>3. The bank account status changes to Disabled<br>4. The linked Pending withdrawal request remains in Pending status — it is not auto-cancelled |

**Test Data:** a Pending fiat withdrawal request to the target bank account

---

### G. Bank Account Enable (7 cases)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Locate a Disabled bank account in the list | *(none stated)* |
| 2 | Open the action menu for that record | 1. An "Enable" action option is visible |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the action buttons in the detail drawer | 1. An "Enable" button is visible and clickable |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Click "Cancel" on the modal | 1. The modal closes<br>2. The bank account remains in Disabled status<br>3. No changes are made |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Click "Enable" on the confirmation modal | *(none stated)* |
| 2 | Observe the result in the list | 1. The modal closes<br>2. Success message: "This bank account has been enabled."<br>3. The bank account status changes to "Active" immediately<br>4. The record moves back to the Active group in the sorted list |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Click Enable on a Disabled bank account | *(none stated)* |
| 2 | Observe the result | 1. Enable is blocked<br>2. Error message displayed: "This member already has 5 active bank accounts. Disable another record before enabling this one."<br>3. The Disabled bank account remains in Disabled status |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Open the member's Withdrawal Accounts tab | 1. Bank Accounts section shows 4 Active records and 2 Disabled records |
| 2 | Click Enable on one of the Disabled bank accounts | 1. Enable confirmation modal appears |
...
### H. Crypto Addresses list (3 cases)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the Crypto Addresses section | 1. The section displays: "This member has no crypto addresses."<br>2. No table rows or loading indicators are shown |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the Crypto Addresses section table columns | 1. The following columns are visible: Asset, Network, Wallet Address (masked), Status, Created Date<br>2. Column headers match the specified labels exactly |
| 2 | Observe the Wallet Address column in the Crypto Addresses list | 1. Wallet Address is shown in masked format: first 6 characters + separator + last 4 characters (e.g. 0x1234...5678)<br>2. The characters between first 6 and last 4 are not visible |
| 3 | Observe the Network column in the Crypto Addresses list | 1. The Network column is visible<br>2. Each row shows the correct network for that crypto address (e.g. USDT on ERC20 shows "ERC20", USDT on TRC20 shows "TRC20") |

**Test Data:** one crypto address on ERC20 and one on TRC20 for the same asset
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the sort order in the Crypto Addresses list | 1. Active records appear at the top, sorted by Created Date descending<br>2. Disabled records appear after all Active records, sorted by Created Date descending<br>3. Deleted records appear last, sorted by Created Date descending |

**Test Data:** none

---

### I. Crypto Address Details drawer (4 cases)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Click on any crypto address record in the list | 1. A read-only detail drawer opens<br>2. The following fields are visible: Label, Asset, Network, Wallet Address (full unmasked), Memo / Tag (supported address), Status, Created Date, Last Updated Date<br>3. All values match the data the member saved |

**Test Data:** a crypto address on an asset/network that supports Memo/Tag (e.g. XRP, XLM)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the Wallet Address field in the detail drawer | 1. The full unmasked Wallet Address is shown<br>2. This differs from the masked format (first 6 + last 4) shown in the list |

**Test Data:** the known full wallet address of the record under test
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the Memo / Tag field area in the detail drawer | 1. The Memo / Tag field is not visible |

**Test Data:** a BTC-Bitcoin crypto address
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Click on a Deleted crypto address record in the list | 1. The detail drawer opens successfully<br>2. All fields are visible in read-only mode<br>3. No Disable or Enable buttons are present<br>4. Status field shows "Deleted" |

**Test Data:** none

---

### J. Crypto Address Disable (6 cases)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Locate an Active crypto address in the list | *(none stated)* |
| 2 | Open the action menu for that record | 1. A "Disable" action option is visible |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Click "Cancel" on the modal | 1. The modal closes<br>2. The crypto address remains in Active status<br>3. No changes are made |
...
|---|--------|-----------------|
| 1 | Click "Disable" on the confirmation modal | *(none stated)* |
| 2 | Observe the result in the list | 1. The modal closes<br>2. Success message: "This crypto address has been disabled."<br>3. The crypto address status changes to "Disabled" immediately<br>4. The record moves to after the Active group in the sorted list<br>5. The Last Updated Date updates to the current timestamp |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Click Disable on the Active crypto address that has a linked Pending withdrawal | *(none stated)* |
| 2 | Confirm the Disable action in the modal | *(none stated)* |
| 3 | Observe the crypto address status and Pending withdrawal status | 1. The Disable action succeeds<br>2. Success message: "This crypto address has been disabled."<br>3. The crypto address status changes to Disabled<br>4. The linked Pending withdrawal remains in Pending status — not auto-cancelled |

**Test Data:** a Pending crypto withdrawal request to the target address

---

### K. Crypto Address Enable (6 cases)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Locate a Disabled crypto address in the list | *(none stated)* |
| 2 | Open the action menu for that record | 1. An "Enable" action option is visible |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Click "Cancel" on the modal | 1. The modal closes<br>2. The crypto address remains in Disabled status<br>3. No changes are made |
...
|---|--------|-----------------|
| 1 | Click "Enable" on the confirmation modal | *(none stated)* |
| 2 | Observe the result in the list | 1. The modal closes<br>2. Success message: "This crypto address has been enabled."<br>3. The crypto address status changes to "Active" immediately<br>4. The record moves back to the Active group in the sorted list |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Click Enable on a Disabled crypto address | *(none stated)* |
| 2 | Observe the result | 1. Enable is blocked<br>2. Error message: "This member already has 20 active crypto addresses. Disable another record before enabling this one."<br>3. The Disabled crypto address remains in Disabled status |
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Admin Disables a withdrawal account record | *(none stated)* |
| 2 | Request BE support to check the audit log for the Disable event | 1. An audit log entry exists for the Disable action<br>2. The entry contains: admin identifier, timestamp, record ID, destination type (bank account / crypto address), and member ID<br>3. The entry is written at the time of the successful Disable (not on Cancel) |

**Test Data:** none

> Requires BE support — the audit log is not exposed on the UI.
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Admin Enables a Disabled withdrawal account record | *(none stated)* |
| 2 | Request BE support to check the audit log for the Enable event | 1. An audit log entry exists for the Enable action<br>2. The entry contains: admin identifier, timestamp, record ID, destination type, and member ID<br>3. No audit entry is created for a cancelled action (admin clicked Cancel on the modal) |
...
### M. API failure handling (2 cases)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Admin confirms Disable in the modal | *(none stated)* |
| 2 | API call returns an error | 1. Error message displayed: "Unable to complete this action. Please try again."<br>2. The record status remains Active (unchanged)<br>3. The modal closes |

**Test Data:** a forced API error response on the Disable endpoint

> Requires request interception or a BE-side fault injection to force the error.
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Admin confirms Enable in the modal | *(none stated)* |
| 2 | API call returns an error | 1. Error message displayed: "Unable to complete this action. Please try again."<br>2. The record status remains Disabled (unchanged) |

**Test Data:** a forced API error response on the Enable endpoint

> Requires request interception or a BE-side fault injection to force the error.

---

### N. Concurrency (3 cases)
...
|---|--------|-----------------|
| 1 | Admin A and Admin B both click "Enable" at approximately the same time | *(none stated)* |
| 2 | Observe the result for both admins | 1. One Enable action succeeds — record becomes Active (member now has 20 Active crypto addresses)<br>2. The second Enable action is rejected<br>3. The second admin sees: "This member already has 20 active crypto addresses. Disable another record before enabling this one."<br>4. The second Disabled record remains in Disabled status |

**Test Data:** member with 19 Active + 2 Disabled crypto addresses; two BO sessions

> Needs two concurrent BO sessions and a member seeded with 19 active addresses.
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Admin A and Admin B both click "Disable" on the same record at approximately the same time | *(none stated)* |
| 2 | Observe the result for both admins and the audit log | 1. The record is Disabled once — the status is Disabled<br>2. One admin sees the success message: "This bank account has been disabled."<br>3. The second admin sees either the same success message (idempotent) or a generic error<br>4. No duplicate audit entries are created for the same event |

**Test Data:** none

> Needs two concurrent BO sessions plus BE support for the audit-log assertion.

---

### O. Balance Approvals — disabled destination (3 cases)

#### TC-162011 · Balance Approvals – Logic – Checker cannot approve withdrawal to Disabled destination
**Priority:** unset | **Automation:** unset | **State:** Done | **Folder:** Balance Approvals (374)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Checker clicks Approve on the Pending withdrawal request | *(none stated)* |
| 2 | Observe the result | 1. The approval is blocked<br>2. A blocking error is displayed: "This withdrawal destination has been disabled and the request cannot be approved. Reject the request to return the funds to the member's available balance."<br>3. The only available action on the request is Reject |

**Test Data:** a Pending withdrawal whose destination has been Disabled
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Checker clicks Reject on the Pending withdrawal request | *(none stated)* |
| 2 | Checker confirms the rejection | 1. The rejection succeeds<br>2. The withdrawal request status changes from Pending to Rejected<br>3. The held amount is returned to the member's available balance |

**Test Data:** member's available balance before rejection, for the return assertion
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Admin Disables the destination bank account / crypto address used by the Pending withdrawal | *(none stated)* |
| 2 | Observe the Pending withdrawal request status in Back Office | 1. The Disable action on the destination succeeds<br>2. The Pending withdrawal request remains in Pending status — it is NOT automatically rejected or cancelled<br>3. The checker must manually action the request |

**Test Data:** none

---

### P. Re-add prevention and app reflection (4 cases)

#### TC-151419 · Withdrawal Accounts – Linked bank accounts – BO disables account immediately reflected in app
**Priority:** unset | **Automation:** unset | **State:** Done | **Folder:** Withdrawal Accounts (3595)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Member fills in the Add Bank Account form using the same Account Number/IBAN and BIC/SWIFT as the Disabled record | *(none stated)* |
| 2 | Member submits the form | 1. The system rejects the submission<br>2. An error message is shown to the member<br>3. No new bank account is created |

**Test Data:** the Account Number/IBAN and BIC/SWIFT of the Disabled record

> Mobile app required. **Expected-result gap:** "An error message is shown" names no string. AO-925 states the member-facing copy lives in AO-923 — the exact text must be sourced before this case can pass or fail on copy.

---

#### TC-162008 · Withdrawal Accounts – Crypto addresses – Add crypto address blocked when matching record is Disabled
**Priority:** unset | **Automation:** unset | **State:** Done | **Folder:** Withdrawal Accounts (3595)
*Also linked to AO-922.*
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Member fills in the Add Crypto Address form using the same Asset, Network, and Wallet Address as the Disabled record | *(none stated)* |
| 2 | Member submits the form | 1. The system rejects the submission<br>2. An error message is shown to the member<br>3. No new crypto address is created |

**Test Data:** the Asset, Network and Wallet Address of the Disabled record

> Mobile app required. **Expected-result gap:** the error string is not named; AO-925 defers the copy to AO-922.
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Go to Account > Withdrawal accounts > Linked bank accounts | 1. Linked bank accounts listing is displayed<br>2. The previously deleted account is no longer visible (or shown as Deleted) |
| 2 | Tap "+" or "Add bank account" | 1. Add bank account form is displayed |
| 3 | Fill in all required fields using the same Account Number/IBAN and BIC/SWIFT as the deleted account | *(none stated)* |
| 4 | Tap "Continue" and proceed through the Review screen | 1. No duplicate/blocked error is shown<br>2. Member is navigated to the Review Bank Account screen |
| 5 | Tap "Save bank account" and complete OTP verification | 1. Bank account is created successfully with Active status<br>2. New account appears in the Linked bank accounts listing<br>3. No error indicating the account already exists |

**Test Data:** the Account Number/IBAN and BIC/SWIFT of the Deleted record; OTP

> Mobile app required, plus OTP access for the member account.

---

### Q. Fiat withdrawal destination picker (2 cases)

#### TC-151523 · Withdraw – Fiat – Bank Account – Disabled accounts shown with Disabled badge and not selectable
**Priority:** unset | **Automation:** unset | **State:** Done | **Folder:** Fiat Withdrawal (3597)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the bank account list | 1. Disabled account is visible in the list, greyed out with a "Disabled" badge |
| 2 | Attempt to tap the Disabled account | 1. Tapping the Disabled account does not select it<br>2. Member remains on the Select bank account screen |

**Test Data:** none

> Mobile app required. **Note:** BR-6 says a Disabled record "is excluded from the withdrawal destination dropdown (AO-881 Req 2)", while this case expects it *shown* with a Disabled badge. Reconcile before executing.
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the re-enabled bank account in the destination list | 1. The re-enabled bank account is no longer greyed-out<br>2. The record is clickable and selectable for withdrawal |
...
### R. Crypto withdrawal destination picker (1 case)

#### TC-162010 · Withdrawal – Crypto – CryptoAddressScreen – Disabled crypto address shown non-clickable
**Priority:** unset | **Automation:** unset | **State:** Done | **Folder:** Crypto Withdrawal (3792)
...
| # | Action | Expected Result |
|---|--------|-----------------|
| 1 | Observe the previously Active crypto address in the destination list | 1. The disabled crypto address is visible but rendered in a disabled/greyed-out state<br>2. Tapping it does not proceed to withdrawal confirmation<br>3. Other Active crypto addresses remain selectable |
...
- The AO-1087 comment reports 62 cases; the Testmo issue link returns **65**. Testmo is the source of truth.
- All three reviewer-requested additions from 2026-08-25 exist: TC-191123, TC-191124, TC-191125.
- No case carries a priority or an automation value.
- Four spec-vs-TC conflicts are flagged inline: TC-161965, TC-161966 (role permissions vs BR-1), TC-161972 (Maker-only unmasking vs BR-3), TC-151523 (badge shown vs BR-6 "excluded from dropdown").
- Two cases have unnamed error strings: TC-162007, TC-162008.
