...
**1.1 Load the ticket.** Path given → read it. Otherwise → `disclose_context("jira-retriever")` for `{KEY}` with `save`, then read `tasks/{KEY}/jira.md`.
...
- T → `disclose_context("collect-testmo-cases")` with `{KEY}`, `save`, `no-gate`.
...
Then `disclose_context("grill-tcs")` with `{KEY}`, `no-gate` — it grills every `new` row in `tc-plan.md` against Q1 source, Q2 negative, Q3 oracle, and hands back READY rows plus any moved to `needs-clarification`.
