...
1. **Read what is already known**, scoped to the screens in hand — **never a file in full**:
   `jq '.["{cache key}"].screens.{screen}' .kiro/locator-cache.json` one key at a time, then **only the
   sections** of `.kiro/domain/flows.md` and the platform's domain file that name those screens. Reading a
...
2. **Confirm the deltas only.** Visit each screen once per platform. A cached target the live screen returns
   ...
   a stale-cache finding: correct `.kiro/locator-cache.json`, add the row, carry it to the gate.
...
- Every stale cache entry is corrected in `.kiro/locator-cache.json`.
