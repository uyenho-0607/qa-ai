...
- **Missing input:** STOP — `exec.md` → disclose_context("manual-exec-design"). Never build one here.

**Required reads:** `.kiro/docs/lessons.md`, `.kiro/steering/playwright-rule.md`
...
On locator failure: apply the Locator Recovery Protocol in `.kiro/steering/playwright-rule.md`, update the Locators Reference table in the exec file, then continue from where the TC stopped.
