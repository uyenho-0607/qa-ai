...
1. **Read what is already known**, scoped to the screens in hand — never a file in full:
   `jq '.["{surface key}"].screens.{screen}' .kiro/locator-cache.json` one key at a time, then the sections
   of `.kiro/domain/flows.md` and the surface's domain file that name those screens. Most are already
   recorded.
2. **Confirm the deltas only.** Visit each screen once per target. A cached target the live screen returns
   ...
   finding: correct the entry in `.kiro/locator-cache.json`, and carry it to the gate.
