...
Shape: `.kiro/platforms/TEMPLATE.md`. Loaded only when a member-app platform is enabled in
`.kiro/steering/project-config.md` § Platforms.

Domain: `.kiro/domain/otc-mobile.md`; login runbook `.kiro/domain/login-flow.md`.
Shared across platforms — password policy, OTP, statuses, decimal precision: `.kiro/domain/otc-shared.md`.
Cached locators: `.kiro/locator-cache.json` § `bfg-otc-app`.
...
|---|---|---|---|
| `ios` | Member app (iOS) | — device | `.kiro/steering/maestro-rule.md` |
| `android` | Member app | — device | `.kiro/steering/maestro-rule.md` |
| `app-web` | Member app (web) | `<FILL_IN>` | `.kiro/steering/playwright-rule.md` |
...
- backend state through the endpoints in `.kiro/locator-cache.json` § `memberApi`, authenticated per that
