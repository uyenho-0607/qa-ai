...
disclose_context("audit-skills") with the named target and `report-only`. Follow it fully. disclose_context("writing-for-agents") for the standard the findings are graded against.

The caller edits `.kiro/` under the user's approval, then runs `python3 sync-kiro.py`.
