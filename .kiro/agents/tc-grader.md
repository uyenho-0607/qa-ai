...
- **Cases** → disclose_context("review-tcs") with `{KEY}`, `no-gate`, and any sheet URL. Run its grading phase in full.
- **`plan`** → disclose_context("grill-tcs") with `{KEY} no-gate` and put every `new` row through its three questions.
