...
**To disable a platform:** set Enabled to `no` with the reason. Its pack is never loaded and no TC is planned against it. **To add one:** write a pack from `.kiro/platforms/TEMPLATE.md` and add a row here.
