...
App under test: `APP_PACKAGE` in `.kiro/steering/project-config.md` § Environment. Read it from there;
...
**Platform reality.** Only Android is verified — see `.kiro/locator-cache.json` §
...
Read `.kiro/locator-cache.json` § `bfg-otc-app` before recon and before a run — `screens` for
identifiers, `android` for commands and measured taps. One key at a time:
`jq '.["bfg-otc-app"].screens.{screen}' .kiro/locator-cache.json`.
...
- `.kiro/locator-cache.json` is the only store.
...
- **A clear costs the one-time setup steps.** After it the app demands the SIT env gate
  ...
  Walkthrough: `.kiro/domain/login-flow.md`.
...
- This repo ships no API client. Verify backend state through the endpoints in
  `.kiro/locator-cache.json` § `api`, authenticated with the bearer token that section names.
